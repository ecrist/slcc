...
export default function WeatherStrip({ forecast, loading }: WeatherStripProps) {
  ...
  if (hours.length === 0) {
    ...
    return (
      <div className="mb-4 flex items-center justify-center gap-3 px-4 py-3 rounded-xl bg-white border border-gray-200 shadow-sm">
        ...
        <div className="text-sm text-gray-700">
          ...
          <span className="ml-2 text-gray-500">· {forecast.precipPctMax}% precip</span>
        </div>
      </div>
    );
  }

  // Highlight the cell matching the current local hour, but only when the
  // selected date IS today — so the green "you are here" marker doesn't
  // appear on tomorrow's forecast.
  const now = new Date();
  const todayStr = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}-${String(now.getDate()).padStart(2, "0")}`;
  const currentHour = forecast.date === todayStr
    ? `${String(now.getHours()).padStart(2, "0")}:00`
    : null;

  return (
    <div className="mb-4 rounded-xl bg-white border border-gray-200 shadow-sm overflow-hidden">
      {/* Header: day summary — centered */}
      <div className="flex items-center justify-center gap-3 px-4 py-3 border-b border-gray-100 bg-gradient-to-b from-gray-50/60 to-white">
        <summary.Icon className={`w-8 h-8 shrink-0 ${summary.color}`} strokeWidth={1.75} aria-hidden />
        <div className="flex items-baseline gap-3 flex-wrap justify-center">
          <span className="text-base font-semibold text-gray-900">{summary.label}</span>
          <span className="text-sm text-gray-700 tabular-nums">
            <span className="font-semibold text-gray-900">{forecast.tempMaxF}°</span>
            <span className="text-gray-400 mx-1">/</span>
            {forecast.tempMinF}°
          </span>
          <span className="text-xs text-gray-500 inline-flex items-center gap-1">
            <Droplets className="w-3.5 h-3.5 text-sky-500" strokeWidth={2} aria-hidden />
            {forecast.precipPctMax}%
          </span>
        </div>
      </div>

      {/* Hourly strip — centered when it fits, scrolls when it doesn't */}
      <div className="overflow-x-auto">
        <div className="flex justify-center min-w-max gap-2 py-3 px-3">
          {hours.map((h) => {
            ...
            return (
              <div
                key={h.time}
                className={`px-3 py-2 flex flex-col items-center gap-1 min-w-[56px] rounded-lg transition-colors ${
                  isNow ? "bg-emerald-50 ring-1 ring-emerald-200" : "hover:bg-gray-50"
                }`}
                title={`${ic.label} · ${h.tempF}°F · ${h.precipPct}% precip${isNow ? " · now" : ""}`}
              >
                <span className={`text-[11px] font-medium tabular-nums leading-none ${isNow ? "text-emerald-700" : "text-gray-500"}`}>
                  {isNow ? "Now" : formatHour(h.time)}
                </span>
                <ic.Icon className={`w-6 h-6 ${ic.color}`} strokeWidth={1.75} aria-hidden />
                <span className="text-base font-semibold text-gray-900 tabular-nums leading-none">{h.tempF}°</span>
                <span className={`text-[10px] tabular-nums leading-none inline-flex items-center gap-0.5 ${wet ? "text-blue-700 font-semibold" : h.precipPct >= 20 ? "text-sky-600" : "text-gray-300"}`}>
                  <Droplets className="w-2.5 h-2.5" strokeWidth={2.25} aria-hidden />
                  {h.precipPct}%
                </span>
              </div>
            );
