...
export default function AnnouncementBanner() {
  ...
  useEffect(() => {
    fetchAnnouncement();
    // Re-fetch when admin saves announcement settings
    const handler = () => fetchAnnouncement();
    window.addEventListener("announcement-updated", handler);
    return () => window.removeEventListener("announcement-updated", handler);
  }, [fetchAnnouncement]);

  if (!message) return null;

  return (
    <div className="bg-swan-gold text-white animate-slide-down">
